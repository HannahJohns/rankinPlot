...
pp_plot <- function(x,
  ...
  p0 <- x$p_1[x$strata==x$strata[1]]
  p1 <- x$p_2[x$strata==x$strata[1]]

  posx <- cbind(score_1 = x$score[x$strata==x$strata[1]], xmin=cumsum(p0)-p0,xmax=cumsum(p0))
  posy <- cbind(score_2 = x$score[x$strata==x$strata[1]], ymin=cumsum(p1)-p1,ymax=cumsum(p1))
  ...
  if(TRUE){
    ...
    allPoints <- allPoints[order(allPoints$score),]
  ...
  this_xtab <- rbind(
    c(sum(x$n_1[as.numeric(x$score)>i & x$strata == x$strata[1]]),
      sum(x$n_1[as.numeric(x$score)<=i & x$strata == x$strata[1]])
      ),
    c(sum(x$n_2[as.numeric(x$score)>i & x$strata == x$strata[1]]),
      sum(x$n_2[as.numeric(x$score)<=i & x$strata == x$strata[1]])
    )
  )
